...
func GGetProvider(Provider pb.Provider) []*pb.GameConfig {
	data := obj.Load().provider
	if value, ok := data.Get(Provider); ok {
		return value
	}
	return nil
}

func GWalkProvider(Provider pb.Provider, f func(*pb.GameConfig) bool) {
	data := obj.Load().provider
	if values, ok := data.Get(Provider); ok {
...
func GGetGameType(GameType pb.GameType) []*pb.GameConfig {
...
func GWalkGameType(GameType pb.GameType, f func(*pb.GameConfig) bool) {
	data := obj.Load().gameType
	if values, ok := data.Get(GameType); ok {
		for _, item := range values {
			if !f(item) {
				return
			}
