...
import (
	...
	"github.com/hechh/framework/pkg/fwatcher/internal/base"
...
func (d *FWatcher) Init(cfg *Config) (err error) {
	...
	// 同步配置到etcd中
	if cfg.IsSync {
		abspath, err := base.EnsureDir(filepath.Join(cfg.XlsxPath, "json"))
		if err != nil {
			return err
		}
		files, err := registry.Glob(fmt.Sprintf("%s/*%s", abspath, cfg.Ext))
		if err != nil {
			return err
		}
		/*
	...
	// 同步etcd配置到本地
	if d.abspath, err = base.EnsureDir(cfg.DataPath); err != nil {
		return
	}
	if err = d.sync.Fetch(d.saveOnly); err != nil {
		return
	}

	// 加载所有本地配置
	files, err := registry.Glob(fmt.Sprintf("%s/*%s", d.abspath, cfg.Ext))
	if err != nil {
		return err
	}
	for sheet, file := range files {
		info := parser.NewFileInfo(sheet, file)
...
func (d *FWatcher) saveOnly(key string, value []byte) {
	sheet := strings.TrimPrefix(key, d.cfg.Etcd.Prefix+"/")
	filename := filepath.Join(d.abspath, sheet+d.cfg.Ext)
	if err := fileutil.AtomicSave(filename, value); err != nil {
