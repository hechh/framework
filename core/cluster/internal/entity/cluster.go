...
type Cluster struct {
	...
	buckets  [domain.CLUSTER_BUCKET_SIZE]*packet.Node
}

func NewCluster(nodeType uint32) domain.ICluster {
	return &Cluster{
		nodeType: nodeType,
...
func (c *Cluster) Del(nodeId uint32) (nn *packet.Node) {
	if nn = c.Get(nodeId); nn != nil {
		...
		ll := len(c.data)
		newVal := math.Ceil(domain.CLUSTER_BUCKET_SIZE / float64(ll-1))
		oldVal := math.Ceil(domain.CLUSTER_BUCKET_SIZE / float64(ll))
...
func (c *Cluster) Add(node *packet.Node) {
	...
	defer c.mutex.Unlock()
	ll := len(c.data)
	newVal := math.Ceil(domain.CLUSTER_BUCKET_SIZE / float64(ll))
	oldVal := math.Ceil(domain.CLUSTER_BUCKET_SIZE / float64(ll+1))
	diff := int(newVal - oldVal)
...
func (c *Cluster) Random(seed uint64) *packet.Node {
	...
	defer c.mutex.RUnlock()
	if seed <= 0 {
		return c.buckets[random.Intn(domain.CLUSTER_BUCKET_SIZE)]
	}
	// hash路由
	h := pool.GetHash64()
	...
	b[7] = byte(seed)
	h.Write(b[:])
	return c.buckets[h.Sum64()%domain.CLUSTER_BUCKET_SIZE]
}
