...
func (d *Actor) Start() bool     { return d.msgs.Start() }
...
func (d *Actor) Register(ac IActor, c redispool.ICache, opts ...msgqueue.Option) {
...
func (d *Actor) RegisterTimer(name string, ms time.Duration, times int32) error {
	task := timer.NewTask(d.msgs.GetIdPointer(), ms, times, func() {
		head := global.GetHead(fun.ACTOR(name, d.msgs.GetId()))
