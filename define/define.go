...
type ILogger interface {
	Trace(skip int, format string, args ...any)
	Debug(skip int, format string, args ...any)
	Warn(skip int, format string, args ...any)
	Info(skip int, format string, args ...any)
	Error(skip int, format string, args ...any)
	Fatal(skip int, format string, args ...any)
	Close()
}

// 定时器接口
type ITimer interface {
	Register(id *uint64, ttl time.Duration, times int32, task func()) error // 注册定时任务
	Close()                                                                 // 关闭定时器服务
}
...
type ICluster interface {
	Size() int               // 集群节点数量
	Add(node INode)          // 添加节点
	Get(nodeId int32) INode  // 获取节点
	Del(nodeId int32) INode  // 删除节点
	Route(seed uint64) INode // 路由一个节点
}
