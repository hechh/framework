...
func (d *EmptyHandler[Actor]) Call(obj any, ctx framework.IContext, args ...any) func() {
	return func() {
		...
		defer func() {
			...
			if err != nil {
				mlog.Errorf("[result] 调用%s耗时%d毫秒, error:%v, head:%v", d.GetName(), endTime-startTime, err, ctx.GetHead())
			} else {
				mlog.Tracef("[result] 调用%s耗时%d毫秒, head:%v", d.GetName(), endTime-startTime, ctx.GetHead())
			}
		}()
