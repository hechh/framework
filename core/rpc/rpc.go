...
type RpcMgr struct {
	names map[tuple.Tuple2[uint32, string]]IRpc
	apis  map[tuple.Tuple2[uint32, uint32]]IRpc
	cmds  map[uint32]IRpc
}

func NewRpcMgr() *RpcMgr {
	return &RpcMgr{
		names: make(map[tuple.Tuple2[uint32, string]]IRpc),
		apis:  make(map[tuple.Tuple2[uint32, uint32]]IRpc),
...
func (m *RpcMgr) GetByActorFunc(nodeType uint32, actorFunc uint32) IRpc {
	if item, ok := m.apis[tuple.T2(nodeType, actorFunc)]; ok {
		return item
	}
	return nil
}
...
func (m *RpcMgr) GetByActorFuncName(nodeType uint32, name string) IRpc {
	if item, ok := m.names[tuple.T2(nodeType, name)]; ok {
		return item
	}
	return nil
}
...
func (m *RpcMgr) GetRpc(head *packet.Head) IRpc {
	if head.ActorFunc > 0 {
		if item, ok := m.apis[tuple.T2(head.DstType, head.ActorFunc)]; ok {
			return item
		}
	}
	if item, ok := m.names[tuple.T2(head.DstType, head.ActorFuncName)]; ok {
		return item
	}
	return nil
}
