...
func (d *ActorMgr) SendMsg(ctx framework.IContext, args ...any) error {
	...
	//mlog.Tracef( "[actor] ActorMgr(%s)本地调用 head:%v, error:%v, args:%v", ctx.GetActorFunc(), ctx.GetHead(), err, args)
	return err
}
...
func (d *ActorMgr) Send(ctx framework.IContext, buf []byte) error {
	...
	//mlog.Tracef( "[actor] ActorMgr(%s)远程调用 head:%v, error:%v, args:%v", ctx.GetActorFunc(), ctx.GetHead(), err, buf)
	return err
}
