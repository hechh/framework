...
func (d *Actor) GetId() uint64   { return d.msgs.GetId() }
func (d *Actor) Start() error    { return d.msgs.Start(mlog.Fatalf) }
...
func (d *Actor) Register(ac IActor, c define.ICache, opts ...queue.Option) {
	name := utils.ParseName(reflect.TypeOf(ac))
	opts = append(opts, queue.WithName(name))
	d.msgs = queue.NewMsgQueue[queue.ITask](opts...)
	d.self = ac
	d.cache = c
}
