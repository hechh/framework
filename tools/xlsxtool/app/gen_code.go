...
import (
	...
	"github.com/hechh/framework/tools/xlsxtool/internal"
...
// keyExpr key 表达式：多 key 用 tplutil.TupleN(...)，单 key 直接返回值
func keyExpr(ref string, idx *domain.Index) string {
	if len(idx.List) > 1 {
		...
		return fmt.Sprintf("tplutil.T%d(%s)", len(parts), strings.Join(parts, ","))
	}
	return idx.GetValue(false, ref)
}

// indexKeyType map 的 key 类型：单 key 为字段类型，多 key 为 tplutil.TupleN[T,...]
func indexKeyType(idx *domain.Index) string {
	if len(idx.List) > 1 {
		...
		return fmt.Sprintf("tplutil.Tuple%d[%s]", len(types), strings.Join(types, ","))
	}
	if idx.List[0] == nil {
		return "interface{}"
	}
