...
type Service struct {
	...
	rpcs   map[uint32]map[uint32]define.IHandler // 远程接口
...
func NewService() *Service {
	return &Service{
		...
		rpcs:   make(map[uint32]map[uint32]define.IHandler),
...
func (d *Service) Register(hh define.IHandler) {
	d.names[hh.GetName()] = hh.GetId()
	d.values[hh.GetId()] = hh.GetName()
	d.locals[hh.GetName()] = hh
}

func (d *Service) RegisterRpc(hh define.IHandler) {
	d.names[hh.GetName()] = hh.GetId()
	d.values[hh.GetId()] = hh.GetName()
	if hh.GetCmd() > 0 {
		d.cmds[hh.GetCmd()] = hh
...
func (d *Service) Get(actorFunc string) define.IHandler {
...
func (d *Service) GetByCmd(cmd uint32) define.IHandler {
...
func (d *Service) GetByRpc(nodeType uint32, id any) define.IHandler {
