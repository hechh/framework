...
func (d *NatsBus) Subscribe(topic string, f func(*packet.Message)) error {
	_, err := d.client.Subscribe(path.Join(d.prefix, topic), func(msg *nats.Msg) {
...
func (d *NatsBus) Send(topic string, body []byte) error {
	return d.client.Publish(path.Join(d.prefix, topic), body)
}

func (d *NatsBus) Request(topic string, body []byte, cb func([]byte) error) error {
	resp, err := d.client.Request(path.Join(d.prefix, topic), body, 3*time.Second)
	if err != nil {
		return err
	}
	return cb(resp.Data)
